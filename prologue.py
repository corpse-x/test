...
#    Update user progress in the database (start or complete prologue)
    try:
        ...
        if user_id not in data:
            data[user_id] = {}  # If the user doesn't exist, create their entry

        if started:
            data[user_id]["started_prologue"] = True
        
        if completed:
            data[user_id]["completed_prologue"] = True
...
def enter(update: Update, context: CallbackContext):
    ...
    if user_data.get("completed_prologue", False):
        # If the user has completed the prologue, notify them
