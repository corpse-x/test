...
DB_SQL="postgresql://postgres:WUcp0oe6jS5DtnUY@tastelessly-delicate-moccasin.data-1.use1.tembo.io:5432/postgres"
...
def start(update: Update, context: CallbackContext):
    ...
    try:
        ...
        is_new_user = add_user_to_database(user)

        if is_new_user:
            log_new_user(update, context)  # Log only if the user is new

        # Send a personalized welcome message
        update.message.reply_text(f"Welcome to the journey, {user.first_name}! 🎉")
